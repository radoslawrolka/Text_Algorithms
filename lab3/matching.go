...
// zamina z map[byte]int na []int
func findLastOccurrencesFast(s []byte) []int {
...
// zamina z map[byte]int na []int
func boyerMooreHasPrefixFast(text, pat []byte,
...
// zamiana z map[byte]int na []int
func BoyerMooreFast(pat, text []byte, output func(int)) {
	lastOccurrences := findLastOccurrencesFast(pat)
	goodSuffixes := simpleComputeGoodSuffixes(pat)
	for i := 0; i+len(pat) <= len(text); /**/ {
		found, shift := boyerMooreHasPrefixFast(text[i:], pat, lastOccurrences, goodSuffixes)
		if found {
			output(i)
		}
		i += shift
