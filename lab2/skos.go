...
VALUES (?,?,?,?,?,?,?,?)`, ToAnySlice(ColumnNames)...)
	...
	for {
		...
		for _, c := range ColumnNames {
			...
			if c == Jednostka && strings.HasPrefix(string(as), "Wydział ") {
				ss = append(ss, AbbreviateFacultyName(string(as), true))
				ss = append(ss, AbbreviateFacultyName(string(as), false))
			}
...
func AbbreviateFacultyName(s string, rmStopwords bool) ASCIIStem {
	var cut = regexp.MustCompile(`, +\pL+[^iy]( |$)`)
	divided := cut.Split(s, -1)[0]
	res2 := ASCIIStringToASCIIStemSlice(ToASCIIString(divided), false)
	abb := ""
	for _, w := range res2 {
		if rmStopwords && len(w) == 1 && w[0] == byte('i') {
			continue
		}
		abb = abb + string(w[0])
	}
	return ASCIIStem(abb)
}
