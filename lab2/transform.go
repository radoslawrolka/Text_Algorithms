...
func TransformPhoneNumbers(s string) string {
	...
	s = phone3222.ReplaceAllString(s, "${2}-${3}${4}-${5}${6}") // Zadanie 1
	return s
}
...
var (
	...
	reDzeG = regexp.MustCompile("^(.+)dz(e|y)$") // Zadanie 3
	// Pasuje do rzeczowników zakończonych na -rze i -rzy,
	// np. "Piotrze" i "doktorzy"
	reRzeR = regexp.MustCompile("^(.+)rz(e|y)$") // Zadanie 4
...
func ToASCIIStems(w ASCIIWord) []ASCIIStem {
	...
	if m := reRzeR.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(m[1]+"r", m[1]+"rz") // Zadanie 4
	}
	if m := reCień.FindStringSubmatch(s); m != nil {
		return ToASCIIStemSlice(s, m[1]+"tn") // Zadanie 5
	}
